...
const createAddress = async function (req, res, next) {
    try {
        ...
        if (!isRegisteredUserId) {
            return res.status(422).send({ status: 1008, message: "This userId does not belongs to any of the delivery partners" })
        }
...
const updateAddress = async function (req, res, next) {
    try {
        ...
        const enteredAddressId = await Address.findOne({ where: { id: addressId, userRole: { [Op.eq]: 'deliverypartner' } } })
...
const getAddress = async function (req, res, next) {
    try {
        ...
        if ("userId" in data) {
            ...
            const isRegisteredUserId = await Address.findOne({ where: { id: userId, userRole: { [Op.eq]: 'deliverypartner' } } });
