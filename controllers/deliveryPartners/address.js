...
const index = async function (req, res) {
    try {
        ...
        if (Object.keys(req.query).length > 0) {
            ...
            if (!findAddressByFilter.length)
                return res.status(404).send({ status: 1006, message: "No Addresses found" })

            return res.status(200).send({ status: 1010, Menu: findAddressByFilter })
        } else {
            let findAllAddress = await Address.findAll({ where: { userRole: "deliverypartner" } })
